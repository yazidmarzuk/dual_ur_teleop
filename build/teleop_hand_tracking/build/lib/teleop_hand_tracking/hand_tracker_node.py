#!/usr/bin/env python3
"""
Hand tracking + teleop: same behavior as /root/hand_teleop but for UR arms.
- Index finger tip → position (lowpass), atan2(y,x) → yaw. Pose: x*2, y*2, z=0.4.
- Twist from (pose - prev_pose)/dt, clamp linear.x/y and angular.z. Gripper from pinch.
"""
import os
import math
import cv2
...
class HandTrackingNode(Node):
    def __init__(self):
        ...
        # Gripper joint limits: lower=-0.03 (open/spread), upper=0.0 (closed)
        self.closed_pos = 0.0
        self.open_pos = -0.03
        self.move_time = 1.0
        self.alpha = 0.5

        # Same topic names as reference (Panda); left=panda1, right=panda2
        ...
        # UR: twist to Servo; gripper to UR gripper controller
        ...
        # Camera: index 0 or VIDEO_SOURCE URL (like reference)
        ...
        # For twist: (pos - prev_pos)/dt, clamp (like reference teleop_moveit)
        ...
        # Twist published in the arm's base frame — matches servo robot_link_command_frame.
        # Same pattern as hand_teleop using left_fr3_link0 / right_fr3_link0 (Franka base frames).
        # No rotation transformation needed: velocity is expressed directly in the base frame.
    ...
    def _publish_twist_from_pose_delta(self, pub, frame_id, pos, euler, prev_time, prev_pos, prev_euler):
        ...
        # Velocity in the arm's base frame — no rotation needed (same as hand_teleop teleop_moveit.py)
